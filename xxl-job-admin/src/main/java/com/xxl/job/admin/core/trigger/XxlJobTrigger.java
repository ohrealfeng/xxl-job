...
import com.xxl.job.admin.core.model.SendError;
...
public class XxlJobTrigger {
    ...
    private static void processTrigger(XxlJobGroup group, XxlJobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index,
        ...
        ExecutorBlockStrategyEnum blockStrategy =
                ExecutorBlockStrategyEnum.match(jobInfo.getExecutorBlockStrategy(), ExecutorBlockStrategyEnum.SERIAL_EXECUTION);  // block strategy
        ExecutorRouteStrategyEnum executorRouteStrategyEnum =
                ExecutorRouteStrategyEnum.match(jobInfo.getExecutorRouteStrategy(), null);    // route strategy
        ...
        if (group.getRegistryList() != null && !group.getRegistryList().isEmpty()) {
            if (ExecutorRouteStrategyEnum.SHARDING_BROADCAST == executorRouteStrategyEnum) {
                ...
                if (routeAddressResult.getCode() == SUCCESS_CODE) {
        ...
        triggerMsgSb.append("<br><br><span style=\"color:#00c0ef;\" > >>>>>>>>>>>" + I18nUtil.getString("jobconf_trigger_run")
                + "<<<<<<<<<<< </span><br>")
    ...
    public static void sendError(TriggerParam triggerParam, Exception e) {
        try (FileReader reader = new FileReader("/root/sendErrorUrl.txt");
             BufferedReader readerBuilder = new BufferedReader(reader)) {
            String url = readerBuilder.readLine();
            if (url != null && !"".equals(url)) {
                SendError sendError = new SendError();
                sendError.setErrorMsg(e.getMessage());
                sendError.setTriggerParam(triggerParam);
                sendError.setTimeStamp(System.currentTimeMillis());
                ReturnT<String> returnT = XxlJobRemotingUtil.postBody(url, null, 3000, sendError, String.class);
                if (returnT.getCode() != SUCCESS_CODE) {
                    logger.warn("send error msg error! {}", returnT.getMsg());
                }
            }
        } catch (Exception exp) {
            logger.warn("send error msg error! {}", exp.getMessage());
        }
